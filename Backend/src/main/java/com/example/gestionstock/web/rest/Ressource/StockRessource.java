...
/*    @GetMapping("/articles/search")
    ...
    //GET http://localhost:8081/api/stock/list?articleId=1&depotId=1
    @GetMapping("/list")
    public List<Stock> getStockBatches(@RequestParam Long articleId, @RequestParam Long depotId) {
        RestPreconditions.checkFound(articleRepository.existsById(articleId), "Article not found");
        RestPreconditions.checkFound(depotRepository.existsById(depotId), "Depot not found");
        return stockService.getStockByArticleAndDepot(articleId, depotId);
    }
